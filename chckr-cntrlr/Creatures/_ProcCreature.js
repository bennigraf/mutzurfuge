...
Creature.prototype.makeWorldMap = function() {
	...
	console.log("worldmaps in cache: ", Object.keys(this.world.worldmapCache).length);
	// check if worldmap is already in cache
	if(this.world.worldmapCache[this.roots[0]+'.'+this.roots[1]+'.'+this.roots[2]]) {
		console.log("!!!!!!!! getting worldmap from cache for ",this.roots[0]+'.'+this.roots[1]+'.'+this.roots[2]);
		this.worldmap = this.world.worldmapCache[this.roots[0]+'.'+this.roots[1]+'.'+this.roots[2]];
	} else {
		...
		this.world.worldmapCache[this.roots[0]+'.'+this.roots[1]+'.'+this.roots[2]] = this.worldMap;
	}
	
	var time2 = process.hrtime();
	time2 = time2[0]+time2[1]/1000000000;
	console.log("world map generation:", time2 - time);
	time = time2;
}
